<?php
    ...
    function checkUser($selection, $table, $value, $id = 0) {
        global $conn;
        $statment = $conn->prepare("SELECT $selection FROM $table WHERE $selection = ? AND CUS_ID != ?");
        $statment->execute(array($value, $id));

        return $statment->rowcount();
    }

    function checkDuplicate($selection, $table, $value, $cond, $id = 0) {
        global $conn;
        $statment = $conn->prepare("SELECT $selection FROM $table WHERE $selection = ? AND $cond != ?");
        $statment->execute(array($value, $id));
    ...
    function getLatest($table, $order, $count) {
        global $conn;
        $statment = $conn->prepare("SELECT TOP($count) * FROM $table ORDER BY $order DESC");
        $statment->execute();
