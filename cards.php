        ...
        if($do == "Manage") {
            ...
            if(isset($_POST['insert'])) {
                ...
                if(empty($errors)){
                    ...
                    try {
                        $max_stmt = $conn->prepare("SELECT MAX(CARD_ID) FROM CARD_INFO");
                        $max_stmt->execute();
                        $max = $max_stmt->fetchColumn();
                        $max = $max + 1;

                        $sql2 = "INSERT INTO CARD_INFO(CARD_ID, CARD_NUMBER, CARD_PIN, CARD_HOLDER, EXPIRES_ON, CARD_TYPE, AMOUNT) VALUES(?, ?, ?, ?, ?, ?, ?)";
                        $stmt2 = $conn->prepare($sql2);
                        $stmt2->execute(array($max, $number, $pin, $holder, $expire, $type, $amount));
                        echo "<script>
            ...
            if(isset($_POST['updateCard'])) {
                ...
                try {
                    $sql2 = "UPDATE CARD_INFO SET AMOUNT = ?
                    WHERE CARD_ID = ?";
                    $stmt2 = $conn->prepare($sql2);
            ...
            if(isset($_POST['deleteCard'])) {
                $id = $_POST['card_id'];
                try {
                    $stmt2 = $conn->prepare("DELETE FROM CARD_INFO WHERE CARD_ID = ?");
                        ...
                        foreach($stmt->fetchAll() as $row): ?>
                            <tr>
                                ...
                                <form action="cards.php" method="POST" class="col-md-6 d-flex justify-content-center align-items-center">
                                    ...
                                    <td><input class="form-control form-control-lg text-center" type="text" name="amount" value="<?php echo round($row['AMOUNT'], 2) ?>"></td>
                                    <td>
